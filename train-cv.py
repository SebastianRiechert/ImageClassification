...
def log_avgs_mlflow(args):
    # get experiment ID
    client = mlflow.tracking.MlflowClient()
    exp_id = client.get_experiment_by_name(args.experiment_name).experiment_id
    # load runs from mlflow
    runs = mlflow.search_runs(exp_id)
    # filter to get metrics
    filter_col = [col for col in runs if col.startswith('metric')]
    # calc mean
    metrics_series = runs[filter_col].mean()
    # clean metric names
    metrics_series = metrics_series.rename(lambda x: x[8:])
    # log
    with mlflow.start_run(run_name='average'):
...
def load_data(args):
    ...
    dataset_train = dataset_factory.create_dataset('CSVDataset',
                                                   args.imgs_path,
                                                   args.data_path if args.data_path else data_paths(args, cv)[0],
    ...
    dataset_test = dataset_factory.create_dataset('CSVDataset',
                                                   args.imgs_path,
                                                   args.data_path if args.data_path else data_paths(args, cv)[1],
...
if __name__ == "__main__":
    ...
    # cv training
    for cv in range(args.nr_cv):
        with mlflow.start_run(run_name=str(cv)):
            main(args, cv=cv)    
    
    log_avgs_mlflow(args)
